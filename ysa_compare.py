...
# Veri setini yükleme
file_path = rf'C:\Users\ardae\OneDrive\Masaüstü\Housing.csv'  # Dosya yolunuza göre güncelleyin
...
def build_model(input_dim, hidden_layers, neurons_per_layer):
    model = Sequential()
    model.add(Dense(neurons_per_layer, input_dim=input_dim, activation='tanh'))
...
plt.subplot(1, 2, 1)
plt.scatter(y_test_20, unscaled_prediction_1_128_80, color='green', edgecolors=(0, 0, 0), alpha=0.6, label='Model 1 (128 Neurons) - %80/20')
plt.scatter(y_test_50, unscaled_prediction_1_128_50, color='orange', edgecolors=(0, 0, 0), alpha=0.6, label='Model 1 (128 Neurons) - %50/50')
plt.scatter(y_test_20, unscaled_prediction_2_128_80, color='blue', edgecolors=(0, 0, 0), alpha=0.6, label='Model 2 (128 Neurons) - %80/20')
plt.scatter(y_test_50, unscaled_prediction_2_128_50, color='red', edgecolors=(0, 0, 0), alpha=0.6, label='Model 2 (128 Neurons) - %50/50')
...
def train_and_evaluate_with_parameters(X_train, y_train, X_test, y_test, model_name, test_size=0.2, epochs=50, hidden_layers=0, neurons_per_layer=128, batch_size=32, validation_split=0.2):
    ...
    plt.annotate(f'R-squared: {r2:.4f} ({int(test_size*100)}% Test Size)', xy=(0.75, 0.8), xycoords='axes fraction', fontsize=12, color='black')
...
train_and_evaluate_with_parameters(X, y, X, y, model_name="yeni_model", test_size=test_size, epochs=epochs, hidden_layers=hidden_layers, neurons_per_layer=neurons_per_layer, batch_size=batch_size, validation_split=validation_split)
...
def train_and_evaluate_rf_with_plot(X_train, y_train, X_test, y_test, model_name):
    ...
    # Doğruluk oranlarını ekleme
    plt.annotate(f'R-squared: {r2:.4f}', xy=(0.75, 0.8), xycoords='axes fraction', fontsize=12, color='black')
